...
static Mat g_srcImage, g_dstImage, g_grayImage, g_maskImage;//定义原始图、目标图、灰度图、掩模图  
static int g_nFillMode = 1;//漫水填充的模式  
static int g_nLowDifference = 20, g_nUpDifference = 20;//负差最大值、正差最大值  
static int g_nConnectivity = 4;//表示floodFill函数标识符低八位的连通值  
static int g_bIsColor = true;//是否为彩色图的标识符布尔值  
static bool g_bUseMask = false;//是否显示掩膜窗口的布尔值  
static int g_nNewMaskVal = 255;//新的重新绘制的像素值  
