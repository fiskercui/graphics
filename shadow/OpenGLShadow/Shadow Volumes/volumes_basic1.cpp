...
// 构造、绘制 Shadow Volume，挤出（extrude）到无穷远
void draw_model_volumes(glm::vec4& lpos, bool light=false)
{
	for(int t=0; t<2; ++t){
		...
		int m = glm::dot(n,l0)>=0 ? 1 : -1; // 是否反转四边形环绕方向
		for(int i=0; i<3; ++i){
			glBegin(GL_POLYGON); // 三个边挤出（extrude）的四边形
				if(light) glNormal3fv(
					&glm::normalize(glm::cross(glm::vec3(tri_far[i]), tri[(i+1)%3]-tri[i]))[0] );
				glVertex3fv(&tri[i][0]);
...
void draw2(const glm::mat4& mat_model, const glm::mat4& mat_view)
{
	...
	glMultMatrixf(&mat_model[0][0]);
		draw_model();
		draw_model_volumes(glm::affineInverse(mat_model)*light_pos, true);

	glMatrixMode(GL_MODELVIEW); glLoadMatrixf(&mat_view[0][0]);
		if(light_pos.w!=0) dlight(0.05f);
...
void draw1(const glm::mat4& mat_model, const glm::mat4& mat_view)
{
	...
	// 正面加1，背面减1
	glStencilOpSeparate(GL_FRONT, GL_KEEP, GL_KEEP, GL_INCR_WRAP); // 改进后
...
void key_u()
{
	...
	unsigned char* data = new unsigned char[w*h]; float* pixs = new float[w*h];
