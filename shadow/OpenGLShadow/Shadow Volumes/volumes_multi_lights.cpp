...
void dlight(glm::vec4& light_pos, float size=0.1f) // 在灯上绘制一个球
{ 
...
void draw2(const glm::mat4& mat_model, const glm::mat4& mat_view)
{
	...
	GLfloat pl[4]={0,0,0, 1};
	for(int l=0; l<lights_num; ++l) glLightfv(GL_LIGHT0+l, GL_POSITION, pl); // 光源位置
	...
	for(int l=0; l<lights_num; ++l){
		if(dis_volumes_modeline){
			...
			for(size_t i=0; i<meshes.size(); ++i){
				...
				glColor4fv(c);
				glPushMatrix(); glMultMatrixf(&meshes_mat[i][0][0]);
					vcg_drawSilhEdges(*meshes[i], glm::affineInverse(mat_model*meshes_mat[i])*light_pos[l]);
				glPopMatrix();
			}
			glLineWidth(_lw);
			...
			for(size_t i=0; i<meshes.size(); ++i){
				...
				draw_model_volumes(*meshes[i],
					glm::affineInverse(mat_model*meshes_mat[i])*light_pos[l], true);
				glPopMatrix();
			}
	...
	glMatrixMode(GL_MODELVIEW); glLoadMatrixf(&mat_view[0][0]);
		for(int l=0; l<lights_num; ++l)if(light_pos[l].w!=0) dlight(light_pos[l], 0.05f);

	if(light_rotate) for(int l=0; l<lights_num; ++l)
		light_pos[l] = glm::rotate(glm::radians(1.0f), glm::vec3(0,1,0)) * light_pos[l];
}
...
void draw1(const glm::mat4& mat_model, const glm::mat4& mat_view)
{
	...
	// ------------------------------------------ 第1遍，渲染环境光，深度值
	// 关闭光源，打开环境光
	GLboolean _li0[lights_num];
	for(int l=0; l<lights_num; ++l){
		_li0[l] = glIsEnabled(GL_LIGHT0+l);
		if(_li0[l]) glDisable(GL_LIGHT0+l);
	}
	glMatrixMode(GL_MODELVIEW); glLoadMatrixf(&mat_view[0][0]);
		draw_world();
	glMultMatrixf(&mat_model[0][0]);
	...
	for(int l=0; l<lights_num; ++l){
		...
		for(size_t i=0; i<meshes.size(); ++i){
			glPushMatrix(); glMultMatrixf(&meshes_mat[i][0][0]);
			draw_model_volumes(*meshes[i], glm::affineInverse(mat_model*meshes_mat[i])*light_pos[l]);
			glPopMatrix();
		}
		// 恢复状态
		if(_li) glEnable(GL_LIGHTING);
		...
		if(_li0[l]) glEnable(GL_LIGHT0+l);
		...
		glMatrixMode(GL_MODELVIEW); glLoadMatrixf(&mat_view[0][0]);
		glLightfv(GL_LIGHT0+l, GL_POSITION, &light_pos[l][0]); // 光源位置
			draw_world();
		glMultMatrixf(&mat_model[0][0]);
			draw_model(); 
		// 恢复状态
...
int main(void) 
{
	...
	glStaff::set_mat_view(
		glm::lookAt( glm::vec3(-4, 16, 16), glm::vec3(0,0,0), glm::vec3(0,1,0) ) );
	...
	//glStaff::load_mat_view("matrix_view_zfail");
	{
		...
		for(int l=0; l<lights_num; ++l) glEnable(GL_LIGHT0+l);

		float gac2[4]={0.15f,0.15f,0.15f,1}; glLightModelfv(GL_LIGHT_MODEL_AMBIENT, gac2);

		light_pos[0] = glm::vec4(5,8,-2, 1);
		light_pos[1] = glm::vec4(-2,8,5, 1);
		light_pos[2] = glm::vec4(-3,8,-3, 1);
	}
